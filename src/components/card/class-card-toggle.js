 ...
 *
 * - 문서 전체에 클릭 이벤트를 위임하여,
 *   .class-card.checkbox-mode 요소가 클릭될 때만 동작.
 * - 클릭 시 is-selected 클래스와 data-checked 속성을 토글합니다.
 * - 접근성(ARIA) 동기화를 위해 .class-card__checkbox 의 aria-checked 값도 갱신.
 *
 * 장점:
 * - 이벤트 위임 방식이므로 동적으로 추가된 카드도 자동 반응.
 * - 특정 래퍼(.class-card-wrap)가 없어도 동작 (document.body 기준).
 */
document.addEventListener("DOMContentLoaded", () => {
  document.body.addEventListener("click", (e) => {
    // 클릭한 요소에서 가장 가까운 class-card 탐색
